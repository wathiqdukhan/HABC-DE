...
% Project Title: Non-dominated Sorting Genetic Algorithm2
...
%------------10-4-2021-
 popm=repmat(empty_individual,nPop,1);
    for k=1:nPop %nMutation
        
        i=randi([1 nPop]);
        p=pop(i);
         %popm(k).Position=p(i)+f*(p(i-1)-p(i-3));
        popm(k).Position=Mutatep(p.Position,p.Position-1,p.Position-2,mu);
...
%    popsc=repmat(empty_individual,nPop/5,1);
%     for i=1:nPop/5
%     
%     popsc(i).Position=sample(nVar);
%     
...
%     for ci = 1 : nel
%         current = costs(ci,:);
%         if any(ismember(costs(ci+1:end,:),current,'rows'))== 0                    
%             uniqueIds = [uniqueIds,ci];
%         end        
%     end
%     
